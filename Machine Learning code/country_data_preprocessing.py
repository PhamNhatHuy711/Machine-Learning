...
print(df.isna().sum())
msno.bar(df)
plt.title('Phân phối các Missing Values', fontsize=20,
fontstyle='oblique')
plt.show()
...
def find_outliers(column):
    ...
    non_numeric_indices = pd.to_numeric(df['child_mort'], errors='coerce').isna()
    non_numeric_values = df.loc[non_numeric_indices, 'child_mort']
    df['child_mort'] = pd.to_numeric(df['child_mort'], errors='coerce')
...
# Gọi hàm find_outliers với cột 'child_mort'
find_outliers(df['child_mort'])
# Trựa quan hóa
def plot_outliers(df):
    ...
    color='red')
    plt.show()
plot_outliers(df['child_mort'])
clean_data = df[(df['child_mort'] < up_lim) & (df['child_mort'] > low_lim)]
print("Minimum salary in USD:{}".format(clean_data['child_mort'].min()))
print("Maximum salary in USD:{}".format(clean_data['child_mort'].max()))
...
# Paint red outlier areas on the boxplot
    ...
    color='red')
    plt.show()
plot_outliers(clean_data['child_mort'])
# #Chuẩn hóa dữ liẹu
...
columns_to_scale = ['life_expec','child_mort', 'exports', 'health', 'imports', 'income', 'inflation', 'total_fer', 'gdpp']

# Chuẩn hóa Min-Max
clean_data[columns_to_scale] = scaler.fit_transform(clean_data[columns_to_scale])
clean_data.to_excel('c:\\Users\\LEGION\\Downloads\\ds_country_preprocessing.xlsx', index=True)